...
const PropertyDetails = () => {
  ...
  const handleContactAction = async (action: 'call' | 'chat') => {
    ...
    // If user is owner, just do the action
    if (user?.id === property.user_id) {
      if (action === 'call' && property.contact_phone) {
    ...
    // If user is logged in, auto-create lead and proceed
    if (user) {
      try {
        // Create lead silently
        const {
          error
        } = await supabase
          .from('leads')
          .insert({
            ...
            status: 'new',
            source: action,
            lead_type: property.campaign_id ? 'paid' : 'organic', // Will be handled by trigger too
            source_page: 'listing_page',
            campaign_id: property.campaign_id || null
          });
        ...
        console.error('Error in auto-lead creation:', err);
      }
      ...
      <div className="flex items-center justify-between px-4 py-3 max-w-full">
        ...
        <div className="flex items-center gap-2">
          {user?.id === property.user_id && (
            <Button variant="ghost" size="icon" onClick={() => navigate(`/add-property?edit=${id}`)} className="hover:bg-accent active:scale-95 transition-transform" aria-label="Edit property">
      ...
      <Card className="p-3 md:p-4">
        {(property.contact_name || property.contact_phone || property.contact_email) && <>
          ...
          <div className="grid grid-cols-2 gap-2">
            {property.contact_phone && <Button size="sm" className="gap-1.5 text-xs md:text-sm" onClick={() => handleContactAction('call')}>
              <Phone className="h-3.5 w-3.5 md:h-4 md:w-4" />
              Call
            </Button>}
            <Button size="sm" variant="outline" className="gap-1.5 text-xs md:text-sm" onClick={() => handleContactAction('chat')}>
